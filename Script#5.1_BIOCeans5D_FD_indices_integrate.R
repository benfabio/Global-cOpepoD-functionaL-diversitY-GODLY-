...
# - load and combine with the BCP variables (NPP, C export, PSD, e ratio etc.); save final ddf
...
colnames(all.fd)
data4pca <- na.omit(all.fd[,c(1:3,21:35,39)])
# log10 transform all vars except slope and eratio; colnames(data4pca)
data4pca[,c(4:17)] <- log10(data4pca[,c(4:17)])
...
# Variance               8.695   5.333   1.031   0.393   0.229   0.125   0.067
# % of var.             54.344  33.330   6.444   2.458   1.428   0.781   0.421
# Cumulative % of var.  54.344  87.674  94.118  96.577  98.005
# Basic plots
plot(pca, axes = c(1,2), choix = "var")
plot(pca, axes = c(2,3), choix = "var")
plot(pca, axes = c(3,4), choix = "var")
...
map1 <- ggplot() + geom_raster(aes(x = x, y = y, fill = PC1), data = data4pca) +
     scale_fill_gradient2(name = "PC1 (54.3%)", low = "#3288bd", mid = "white", high = "#b2182b") +
...
map2 <- ggplot() + geom_raster(aes(x = x, y = y, fill = PC2), data = data4pca) +
     scale_fill_gradient2(name = "PC2 (33.3%)", low = "#3288bd", mid = "white", high = "#b2182b") +
...
map3 <- ggplot() + geom_raster(aes(x = x, y = y, fill = PC3), data = data4pca) +
     scale_fill_gradient2(name = "PC3 (6.4%)", low = "#3288bd", mid = "white", high = "#b2182b") +
...
panel <- ggarrange(map1,map2,map3, align = "hv", ncol = 1)
setwd("/net/kryo/work/fabioben/GODLY/plots")
ggsave(plot = panel, filename = "map_PC1-3_BCP_indices.pdf", dpi = 300, height = 10, width = 8)
