...
mclapply(files, function(f) {
        ...
        if( length(mean.beta.jac) == length(comm_fdiv_PA[,"x"]) ) {
            ...
            # ggplot() + geom_raster(aes(x = x, y = y, fill = beta.jne/beta.jac), data = dat) + scale_fill_viridis(name = "ßratio") +
#                      geom_contour(colour = "grey30", binwidth = .1, size = 0.25, aes(x = x, y = y, z = beta.jne/beta.jac), data = dat) +
